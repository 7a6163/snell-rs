...
#   ctx/amd64/snell-server  ctx/amd64/snell-client
#   ctx/arm64/snell-server  ctx/arm64/snell-client
# Both are statically linked musl binaries (no runtime deps, ~1 MB each).
#
...
ARG TARGETARCH
COPY ctx/${TARGETARCH}/snell-server /snell-server
COPY ctx/${TARGETARCH}/snell-client /snell-client
